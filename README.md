...
## 🧠 Arquitetura de Memória & Contexto Permanente para IA (`memoria_projeto/`)

Este repositório inclui um sistema de **memória permanente em arquivos** na pasta `memoria_projeto/`. Ele permite que qualquer Inteligência Artificial (Gemini, Antigravity, Claude, ChatGPT, Cursor) ou desenvolvedor leia o contexto completo do projeto e continue a partir de onde o trabalho parou sem perda de histórico:

* **[memoria_projeto/000_CONTEXTO_GLOBAL_E_HISTORICO.md](file:///c:/finder_logs/memoria_projeto/000_CONTEXTO_GLOBAL_E_HISTORICO.md):** Histórico completo de versões, caminhos de rede SMB, regras de negócio e anatomia dos logs.
* **[memoria_projeto/001_REGRAS_DE_DESENVOLVIMENTO_AI.md](file:///c:/finder_logs/memoria_projeto/001_REGRAS_DE_DESENVOLVIMENTO_AI.md):** Manual de diretrizes de código, padrões de testes e protocolo de atualização de versão/build.
...
+---------------------------------------------------------------------------------------+
| [👁️ Mostrar Apenas Falhas]  [📋 Copiar Diagnóstico]  [📄 Salvar Laudo (.txt)]        |
+---------------------------------------------------------------------------------------+
...
* **Leitura Automática de Erros:** O parser dedicado analisa a anatomia do log carregado e exibe no topo da tela um Card de Diagnóstico contendo:
  ...
  * **Agrupamento Inteligente:** Agrupa sub-blocos do mesmo CI (`U109 (3 blocos)`) no Card sem alterar o texto bruto do log.
...
### ⚡ 3. Realce de Sintaxe C++ Nativo (`QSyntaxHighlighter`)
* Realce de sintaxe nativo sem atrasos de renderização (0ms de overhead) destacando por cores:
  * 💖 **Circuitos Integrados / CIs (`PU...`, `U...`):** Rosa/Magenta.
  * 💙 **Capacitores (`CC...`, `PC...`, `C...`):** Azul.
  * 🧡 **Resistores (`PR...`, `RE...`, `R...`):** Laranja.
  * 💜 **Transistores / FETs (`PQ...`, `Q...`):** Roxo.
  * 💚 **Connectores / Devices (`JTP...`, `J...`):** Verde Água.
  * 🔴 **Falhas & Curtos (`FAIL`, `Short`, `Failed Open`):** Fundo vermelho.
  * 🟡 **Serial Pesquisado:** Amarelo vibrante.

### 👁️ 4. Filtro Alternável "Mostrar Apenas Falhas"
...
### 📋 5. Laudo em 1 Clique & Exportação Formal
* **Copiar Diagnóstico (`📋 Copiar Diagnóstico`):** Copia laudo limpo para Teams/WhatsApp.
* **Salvar Laudo em Arquivo (`📄 Salvar Laudo (.txt)`):** Exporta um relatório técnico formal para arquivamento ou chamados.
...
* Exibição visual de status no rodapé (`🟢 Rede Online` / `🔴 Rede Offline`) evitando travamentos por desconexão de rede no Windows.
...
├── build_release.py       # Automador de Compilação PyInstaller e Releases Versionadas
├── build_exe.bat          # Script em Lote Windows para Executar build_release.py
├── style.qss              # Design System e Estilização CSS Moderna
├── memoria_projeto/       # 🧠 BASE DE CONTEXTO E MEMÓRIA PERMANENTE DO PROJETO PARA IA
│   ├── 000_CONTEXTO_GLOBAL_E_HISTORICO.md
│   └── 001_REGRAS_DE_DESENVOLVIMENTO_AI.md
├── releases/              # Pasta de Armazenamento de Executáveis Versionados (no .gitignore)
└── base de conhecimento/  # Base de amostragem de logs reais de produção (ICT01 e ICT02)
```
...
### Compilar e Gerar Release Versionada na pasta `releases/`
```cmd
build_exe.bat
```
O executável final com versionamento **`ICT_Master_Suite_v6.1.0.exe`** será gravado em `releases/`, e uma cópia principal `ICT_Master_Suite.exe` será mantida na raiz.
